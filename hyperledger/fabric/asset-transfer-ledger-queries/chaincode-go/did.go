...
func (dcc *DIDChaincode) IssueEmployeeDID(ctx contractapi.TransactionContextInterface, id string) error {
	...
	employee.DID = "did:example:" + id // 임시로 DID를 생성하는 방식으로 작성
...
func (dcc *DIDChaincode) VerifyEmployeeDID(ctx contractapi.TransactionContextInterface, id string, did string) (bool, error) {
	employeeJSON, err := ctx.GetStub().GetState(id)
	if err != nil {
		return false, fmt.Errorf("failed to read from world state: %v", err)
	}
	if employeeJSON == nil {
		return false, fmt.Errorf("the employee %s does not exist", id)
	}

	employee := new(Employee)
	err = json.Unmarshal(employeeJSON, employee)
	if err != nil {
		return false, fmt.Errorf("failed to unmarshal employee JSON: %v", err)
	}
